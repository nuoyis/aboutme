...
?>";
        $con1=new yuDB($db_host,$db_user,$db_pwd,$db_name);
		if($con1->yudetect()["code"]!="200"){
			if($con1->yudetect()["dberrorcode"]==2002)
				echo '<div class="alert alert-warning">连接数据库失败，数据库地址填写错误！</div>';
			elseif($con1->yudetect()["dberrorcode"]==1045)
				echo '<div class="alert alert-warning">连接数据库失败，数据库用户名或密码填写错误！</div>';
			elseif($con1->yudetect()["dberrorcode"]==1049)
				echo '<div class="alert alert-warning">连接数据库失败，数据库名不存在！</div>';
			else
				echo '<div class="alert alert-warning">连接数据库失败，['.$con1->yudetect()["dberrorcode"].']'.$con1->yudetect()["error"].'</div>';
		}elseif(file_put_contents('../nuoyis_about/config.php',$config)){
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if($con1->getquery("select * form yuip where 1")["errorcode"]!="00000")
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过诺依阁的个人介绍系统</div>
				<div class="list-group-item">
...
if(!$yudbconfig['dbuser']||!$yudbconfig['dbpwd']||!$yudbconfig['dbname']) {
	...
	$sql=explode(';',$sql);
	$con3=new yuDB($yudbconfig['dbhost'],$yudbconfig['dbuser'],$yudbconfig['dbpwd'],$yudbconfig['dbname']);
	if ($con3->yudetect()["code"]!="200") die('err:'.$con3->yudetect()["error"]);
	$t=0; $e=0; $error='';
	for($i=0;$i<count($sql);$i++) {
		if ($sql[$i]=='')continue;
		if($con3->getquery($sql[$i])["errorcode"]=="00000") {
			++$t;
		} else {
			++$e;
			$error.=$con3->getquery($sql[$i])["error"].'<br/>';
		}
	}
}
if($e==0) {
